...
def main():
    ...
    driver.find_element(By.XPATH, "/html/body/nav/div/ul/li[6]/a").click()

    # Checks the categories page has been reloaded
    assert "Categories Page" in driver.title

    # CLicks on the add category button
    driver.find_element(By.XPATH, "/html/body/div[1]/a").click()

    # Checks the page has been loaded
    assert "Add Category" in driver.title

    # Text input
    category_name = "Test category"
    enter_text("/html/body/form/section/div/div/div/div/div/div[1]/input", category_name) # Category name
